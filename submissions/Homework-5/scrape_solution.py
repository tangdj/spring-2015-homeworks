...
def parse_hotellist_page(html):
  ...
  #          log.info("Next url is %s" % href['href'])
    return hrefs[len(hrefs)-1]['href']

def get_hotel_more_info(html,city,state,datadir="data/"):
    soup=BeautifulSoup(html)
    div=soup.findAll('span', {'class' :'more'})
    #hrefs=div.findAll('a',href=True)
    hrefs=[]
    for h in div:
        for href in h.findAll('a',href=True):
    ...
    for href in hrefs:
        ...
        headers = { 'User-Agent' : user_agent }
        response = requests.get(url, headers=headers)
        html_hotel = response.text.encode('utf-8')
        hotel_name=href.split('-')[4].split('.')[0]
        s=hotel_name+","
        print hotel_name
        ss=[""]
        summary=0
        review=0
        c=5
        soup=BeautifulSoup(html_hotel)
        div=soup.find('ul',{'class':'barChart'})
        for url in div.findAll('span',{'class':'compositeCount'}):
            temp=url.text.split(",")
            print temp
            if(len(temp)==2):
              ss[0]=temp[0]+temp[1]
        ...
        for image in div.findAll('span',{'class':'rate sprite-rating_s rating_s'}):
            draw=image.find('img')
            if draw is not None:
                s=s+draw['alt'].split(" ")[0]+","
        s=s[:-1]+"\n"
        print s
        if not os.path.exists('bihotel.txt'):
           file('bihotel.txt', 'a').close()
        my_file = open("bihotel.txt", "a")
        my_file.write(s)
        #with open(os.path.join(datadir, city + hotel_name + '.html'), "w") as h:
        #   h.write(html_hotel)
...
def scrape_hotels(city, state, datadir='data/'):
    ...
    while(True):
        ...
        get_hotel_more_info(html,city,state,datadir)
        city_url = parse_hotellist_page(html)
  #  html = get_hotellist_page(city_url, 0, city, datadir)
  #  get_hotel_more_info(html,city,state,datadir)
